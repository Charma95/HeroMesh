...
#include <sstream>
#include <vector>
#include <zlib.h>   // Install zlib nuGet package
#include <iostream>
#include <stdexcept>
...
namespace HeroMesh
{
	...
	{
		HEROMESH_LOG_DEBUG("[IO] saveArrayOfArraysToFile: " << fileName);
	...
	{
		HEROMESH_LOG_DEBUG("[IO] loadArrayOfArraysFromFile: " << fileName);
		FILE* f = fopen(fileName.c_str(), "rb");
		if (f == nullptr)
		{
			HEROMESH_THROW_ERROR("[IO] loadArrayOfArraysFromFile: can't open file " << fileName);
		}
		...
		{
			fclose(f);
			HEROMESH_THROW_ERROR("[IO] loadArrayOfArraysFromFile: can't read outer array size");
		}
		...
		for (int i = 0; i < n; i++)
		{
			...
			{
				fclose(f);
				HEROMESH_THROW_ERROR("[IO] loadArrayOfArraysFromFile: can't read inner array size");
			}
			if (m > 0)
			{
				...
				{
					fclose(f);
					HEROMESH_THROW_ERROR("[IO] loadArrayOfArraysFromFile: can't read vector element");
				}
				(*aa)[i] = a;
			};
		};
	...
	void saveArrayToFile(const std::string& fileName, const StaticVector<T>* a, bool docompress = true)
	{
		HEROMESH_LOG_DEBUG("[IO] saveArrayToFile: " << fileName);
		...
		if (!a)
		{
			HEROMESH_LOG_DEBUG("[IO] saveArrayToFile called with NULL static vector");
			return;
		}

		if (a->size() == 0)
		{
			HEROMESH_LOG_WARNING("[IO] saveArrayToFile called with 0-sized static vector");
			return;
		}
		...
		if ((docompress == false) || (a->size() < 1000))
		{
			...
			if (f == NULL)
			{
				HEROMESH_THROW_ERROR("[IO] file " << fileName << " could not be opened, msg: " << strerror(errno));
			}
			int n = a->size();
			if (n == 0)
			{
			...
			{
				fclose(f);
				HEROMESH_THROW_ERROR("[IO] failed to write 1 int to " << fileName << ", msg: " << strerror(errno));
			}
			items = fwrite(&(*a)[0], sizeof(T), n, f);
			if (items < n && ferror(f) != 0)
			{
				fclose(f);
				HEROMESH_THROW_ERROR("[IO] failed to write n items to " << fileName << ", msg: " << strerror(errno));
			}
			fclose(f);
		}
		else
		{
			...
			*/
			...
			if (err != Z_OK)
			{
				HEROMESH_LOG_ERROR("compress error " << err << " : " << (sizeof(T) * a->size()) << " -> " << comprLen << ", n " << a->size());
			...
			else
			{
				...
				if (items < 1 && ferror(f) != 0)
				{
					fclose(f);
					free(compr);
					HEROMESH_THROW_ERROR("[IO] failed to write 1 uLong to " << fileName << ", msg: " << strerror(errno));
				}
				...
				if (items < 1 && ferror(f) != 0)
				{
					fclose(f);
					free(compr);
					HEROMESH_THROW_ERROR("[IO] failed to write " << comprLen << " items to " << fileName << ", msg: " << strerror(errno));
				}
				fclose(f);
			};
	...
	{
		HEROMESH_LOG_DEBUG("[IO] loadArrayFromFile: " << fileName);

		FILE* f = fopen(fileName.c_str(), "rb");
		if (f == NULL)
		{
			HEROMESH_THROW_ERROR("loadArrayFromFile : can't open file " << fileName);
		}
		else
		{
			int n = 0;
			...
			{
				fclose(f);
				HEROMESH_THROW_ERROR("[IO] loadArrayFromFile: can't read array size (1) from " << fileName);
			}
			...
			if (n == -1)
			{
				...
				{
					fclose(f);
					HEROMESH_THROW_ERROR("[IO] loadArrayFromFile: can't read array size (2)");
				}
				...
				if (retval != 1)
				{
					delete a;
					fclose(f);
					HEROMESH_THROW_ERROR("[IO] loadArrayFromFile: can't read ulong elem size");
				}
				Byte* compr = (Byte*)calloc((uInt)comprLen, 1);
				...
				if (retval != comprLen)
				{
					delete a;
					fclose(f);
					HEROMESH_THROW_ERROR("[IO] loadArrayFromFile: can't read blob");
				}
				...
				if (err != Z_OK)
				{
					delete a;
					fclose(f);
					HEROMESH_THROW_ERROR("uncompress error " << err << " : " << (sizeof(T) * n) << " -> " << uncomprLen << ", n " << n);
				}
				...
				if (uncomprLen != sizeof(T) * n)
				{
					delete a;
					fclose(f);
					HEROMESH_THROW_ERROR("loadArrayFromFile: uncompression failed uncomprLen!=sizeof(T)*n");
				}
			...
			else
			{
				...
				if (retval != n)
				{
					delete a;
					fclose(f);
					HEROMESH_THROW_ERROR("[IO] loadArrayFromFile: can't read n elements");
				}
			}
	...
	bool loadArrayFromFile(StaticVector<T>& out, const std::string& fileName, bool printfWarning = false)
	{
		...
		else
		{
			...
			size_t retval = fread(&n, sizeof(int), 1, f);
			if (retval != 1)
				HEROMESH_LOG_WARNING("[IO] loadArrayFromFile: can't read array size (1) from " << fileName);
			out.clear();
			...
			if (n == -1)
			{
				retval = fread(&n, sizeof(int), 1, f);
				if (retval != 1)
					HEROMESH_LOG_WARNING("[IO] loadArrayFromFile: can't read array size (2)");
				out.resize(n);

				uLong comprLen;
				retval = fread(&comprLen, sizeof(uLong), 1, f);
				if (retval != 1)
					HEROMESH_LOG_WARNING("[IO] loadArrayFromFile: can't read ulong elem size");
				Byte* compr = (Byte*)calloc((uInt)comprLen, 1);
				retval = fread(compr, sizeof(Byte), comprLen, f);
				if (retval != comprLen)
					HEROMESH_LOG_WARNING("[IO] loadArrayFromFile: can't read blob");
				...
				if (err != Z_OK)
				{
					HEROMESH_LOG_ERROR("uncompress error " << err << " : " << (sizeof(T) * n) << " -> " << uncomprLen << ", n " << n);
				}
			...
			else
			{
				...
				if (retval != n)
					HEROMESH_LOG_WARNING("[IO] loadArrayFromFile: can't read n elements");
			}
	...
	{
		HEROMESH_LOG_DEBUG("[IO] loadArrayFromFileIntoArray: " << fileName);

		FILE* f = fopen(fileName.c_str(), "rb");
		if (f == NULL)
		{
			HEROMESH_THROW_ERROR("loadArrayFromFileIntoArray: can not open file: " << fileName);
		}
		int n = 0;
		...
		if (n == -1)
		{
			...
			{
				fclose(f);
				HEROMESH_THROW_ERROR("loadArrayFromFileIntoArray: expected length " << a->size() << " loaded length " << n);
			}
			...
			{
				fclose(f);
				HEROMESH_THROW_ERROR("loadArrayFromFileIntoArray: uncompression failed uncomprLen!=sizeof(T)*n");
			}
